...
14
15
16 // Primary RocksDB instance DBOptions db_options ; db_options . compaction_service = std :: make_shared < MyCompactionService > (); // Remote worker OpenAndCompactOptions options ; options . allow_resumption = true ; std :: string result ; Status s = DB :: OpenAndCompact ( options , db_path , // source database path output_directory , // where output SSTs and progress are stored compaction_input , // serialized CompactionServiceInput & result , // serialized CompactionServiceResult override_options ); Future Work Today this feature targets remote compaction. The same checkpoint-and-resume mechanism could also support local compaction after a crash. The core persistence and resume logic is already in CompactionJob ; the remaining work is to integrate it with local compaction scheduling and recovery.
